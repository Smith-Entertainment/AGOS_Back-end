...
public class BairroController {
    ...
    @GetMapping("findBy")
    public ResponseEntity<BairroDTO> findByNome(@RequestParam("nome") final String nome){
        try {
            BairroDTO bairroDTO = this.bairroService.findByNome(nome);
            return ResponseEntity.ok(bairroDTO);
        }catch (IllegalArgumentException e){
            return ResponseEntity.badRequest().body(new BairroDTO());
        }
    ...
    @PostMapping
    public ResponseEntity<BairroDTO> create(@RequestBody final BairroDTO bairroDTO) {
        try {
            BairroDTO createdBairroDTO = this.bairroService.create(bairroDTO);
            return ResponseEntity.ok(createdBairroDTO);
        } catch (IllegalArgumentException e) {
